...
var noFiles = fp.reject(function (node) {
  return node.isFile;
});
var noDirs = fp.reject(function (node) {
  return node.isDirectory;
});
var noDotfiles = fp.reject(function (node) {
...
function () {
  function ProgramVault(options) {
    ...
    if (windowExists && "nw" in window) {
      var nwRequire = fp.get("nw.require", window);
  ...
  _createClass(ProgramVault, [{
    ...
    value: function hasSetupInfo(relpath) {
      try {
        ...
        return _await(_this6.getDirs(relpath), function (_this5$getDirs) {
          return fp.any(isSetupNode, _this5$getDirs);
        });
      } catch (e) {
        return Promise.reject(e);
      }
    ...
    value: function getContents(relpath) {
      try {
        ...
        var _flow2 = fp.flow([fp.map(function (dirent) {
          return _this10.createFsNode(dirent);
        }), fp.reject(function (node) {
    ...
    value: function getFiles() {
      ...
      try {
        ...
        return _await(_this14.getContents(), function (_this13$getContents) {
          ...
          return options.dotfiles === false ? noDotfiles(files) : "onlyExt" in options ? fp.filter(function (node) {
          ...
          return function () {
            if (isDirectory) {
              return _await(_this15.getDirs(relpath), function (_this15$getDirs) {
                var hasSetupInfo = fp.any(isSetupNode, _this15$getDirs);
